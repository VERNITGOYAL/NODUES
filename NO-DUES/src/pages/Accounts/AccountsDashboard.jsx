...
// Reusing these components as they are consistent across administrative departments
...
const AccountsDashboard = () => {
  ...
  // --- 1. Fetch Accounts-Specific Pending Applications ---
  const fetchApplications = async () => {
    setIsLoading(true);
    try {
      // The API returns pending applications specific to the "Accounts" role
      ...
      const mappedApplications = Array.isArray(data)
        ? data.map(app => {
            // Standardize "in_progress" to "Pending" for the Finance/Accounts Admin view
  ...
  // --- 2. Fetch Detailed Application Info (Dues & Fees check) ---
  ...
  // --- 3. Handle Accounts Approval/Rejection ---
  const handleAccountsAction = async (application, action, remarksIn) => {
    if (!application) return;
    setActionError('');
    
    if (action === 'reject' && (!remarksIn || !remarksIn.trim())) {
      setActionError('Remarks/Reason for outstanding dues is required');
      return;
    }
  
    const stageId = application?.active_stage?.stage_id;
    if (!stageId) return setActionError('No actionable stage found for Accounts clearance.');
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Accounts action failed: ${res.status}`);
  
      const newStatus = action === 'approve' ? 'Approved' : 'Rejected';
      
      // Update local state to remove the processed entry
      ...
      setActionError(err?.message || 'Error processing Accounts clearance');
    } finally {
    ...
    <div className="flex h-screen bg-gray-100 font-sans">
          ...
          <motion.div initial="hidden" animate="visible" variants={containerVariants}>
            ...
            <motion.div variants={itemVariants}>
              <h1 className="text-3xl font-extrabold text-gray-900">
                {user?.department_name || 'Accounts'} Section
              </h1>
      ...
      {selectedApplication && (
        <ApplicationActionModal 
          ...
          onAction={handleAccountsAction} 
          actionLoading={actionLoading} 
          actionError={actionError}
          userSchoolName={user?.department_name || 'Accounts Section'}
        />
      )}
    </div>
  );
